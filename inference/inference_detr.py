...
class DetrProcessor:
    def __init__(self, model_path="../detr_api/model.safetensors", config_path=None, confidence_threshold=0.5):
        self.confidence_threshold = confidence_threshold
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"Loading model on {self.device}...")

        config = AutoConfig.from_pretrained("facebook/detr-resnet-50")
        config.num_labels = 3  # Set number of classes to 3
        ...
        # Update the labels
        self.model.config.id2label = {0: "LABEL_0", 1: "api", 2: "asap"}
        self.model.config.label2id = {"LABEL_0": 0, "api": 1, "asap": 2}
    ...
    def process_frame(self, frame):
        ...
        color_map = {
            "api": (0, 0, 255),    # Red in BGR
            "asap": (0, 255, 0),   # Green in BGR
            "LABEL_0": (255, 0, 0) # Blue in BGR (for any other class)
        }
